...
if look == "1":
    ...
    dir_name = 'transfer_0dot8'
    head_path = PATH + dir_name + '/'
    src_name = "_scr_" + dir_name
    time = 0
    for i in Gesture:
        for j in range(2, 4):
            tmp_path = head_path + i + "/time" + str(j) + "/"
            title = i + "/time" + str(j) + "/"+dir_name
            path = tmp_path
            out_cam_p = np.load(path + 'out_cam_p'+ src_name +'.npy', allow_pickle=True)
            radar_larry_src = np.load(path + 'out_radar_p'+ src_name +'.npy', allow_pickle=True)
            # radar_src = np.load(path + 'out_radar_p'+ src_name +'.npy', allow_pickle=True)
            out_radar_p = np.load(path + 'out_center_p'+ src_name +'.npy', allow_pickle=True)
            zeropd = 0
            zeropd1 = 0
