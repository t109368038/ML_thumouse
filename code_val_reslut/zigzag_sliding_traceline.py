...
model_name= "transfer_0dot5_zigzag_epoch400"
for i in Gesture:
    for j in range(2, 4):
        head_path = "D:/thumouse_training_data/zigzag_model/"
        path = 'D:/thumouse_training_data/'+ name +'/zigzag_data/'
        out_cam_p = np.load(path + i + "_time" + str(j) + "_cam.npy", allow_pickle=True)
        out_radar_p = np.load(path + i + "_time" + str(j) + "_radar_zigzag.npy", allow_pickle=True)
        if timesteps == 3:
        ...
        #   if model is CRNN
        # out_radar_p = np.expand_dims(out_radar_p, axis=2)

        # model = keras.models.load_model(head_path+model_name+".h5")
        model = keras.models.load_model("D:\\pythonProject\\ML_thumouse\\transfer_0dot5_zigzag_RNN.h5")
