...
for i in Gesture:
    for j in range(2, 4):
        head_path = "D:/thumouse_training_data/"
        ...
        out_radar_p = np.load(path + i + "_time" + str(j) + "_radar.npy", allow_pickle=True)
        ...
        #
        # model = keras.models.load_model("D:\\pythonProject\\ML_thumouse\\New_model_adam\\light_sliding_normalsrc_timestep3.h5")
        model = keras.models.load_model("D:/thumouse_training_data/old_model_before1100630/light_model_h5/light_sliding_nomralsrc_timestep3.h5")
        # model = keras.models.load_model(head_path+"model/"+name +".h5")
        ...
        # title = str(i) + "/times"+str(j)+'/'+name
        title = str(i) + "/times"+str(j)+'/'+"light_sliding_nomralsrc_timestep3"
