    ...
    for data_set in dataset:
        ...
        for i in Gesture:
            for j in range(2, 4):
                ...
                path = 'D:/thumouse_training_data/'+ data_set +'/sliding_data/'
                head_path = "D:/thumouse_training_data/dict_model/"
                out_cam_p = np.load(path + i + "_time" + str(j) + "_cam.npy", allow_pickle=True)
                out_radar_p = np.load(path + i + "_time" + str(j) + "_radar_dict.npy", allow_pickle=True)
