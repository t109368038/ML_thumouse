...
def build_CRNN_model():
    input_shape = (3,1,45)
    model = Sequential()
    model.add(TimeDistributed(Conv1D(filters=64, kernel_size=45, activation="relu",padding="same",strides=1 ,input_shape=(None, None, 45)),input_shape=input_shape))
...
def build_RNN_model():
    model = Sequential()
    model.add(LSTM(units=32, input_shape=(timesteps, 45), return_sequences=True, kernel_initializer='random_uniform'))
...
def load_slding_data(timesteps):
    ...
    for i in Gesture:
        for j in range(2, 4):
            path = "D:/thumouse_training_data/transfer_0dot5/sliding_data/"
            cam_voxel = np.load(path+ i +"_time" + str(j) + "_cam.npy", allow_pickle=True)
            radar_voxel = np.load(path+ i +"_time" + str(j) + "_radar_xxyyzz.npy", allow_pickle=True)
